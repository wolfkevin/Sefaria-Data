...
depth = 2
with codecs.open('summary_new' + '.tsv', 'wb+', 'utf-8') as csvfile:
    csvfile.write(u'Text\t# of Links Added\tVersion\n')
    post = False
    # for depth in range(1, 4):
    for title in IndexSet({"$and": [{"categories": "Talmud"}, {"categories": "Commentary"}, {"schema.depth": depth}, {"title": "Shita Mekubetzet on Ketubot"}]}):
            text_changed = False
            links_added = 0
            ref = Ref(title.title)
            ...
            if text_changed and post:
                try:
                    ...
                    print post_text(title.title, text_version)
